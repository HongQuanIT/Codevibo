...
class GeneratePress_Block_Elements {
	...
	public function get_dynamic_url( $link_type, $source, $block ) {
		...
		if ( 'term-meta' === $link_type ) {
			$url = get_post_meta( get_queried_object_id(), $block['attrs']['gpDynamicTextCustomField'], true );
		}
		...
		if ( 'generateblocks/headline' === $block['blockName'] || 'generateblocks/button' === $block['blockName'] ) {
			if ( ! empty( $block['attrs']['gpDynamicTextType'] ) && ! empty( $block['attrs']['gpDynamicTextReplace'] ) ) {
				...
				if ( 'title' === $text_type ) {
					...
					$post_title = apply_filters( 'generate_dynamic_text_title', $post_title, $block );
				...
				if ( 'post-date' === $text_type ) {
					...
					$post_date = apply_filters( 'generate_dynamic_text_post_date', $post_date, $block );
				...
				if ( 'post-author' === $text_type ) {
					...
					$post_author = apply_filters( 'generate_dynamic_text_post_author', $post_author, $block );
				...
				if ( 'terms' === $text_type && 'generateblocks/headline' === $block['blockName'] ) {
					if ( ! empty( $block['attrs']['gpDynamicTextTaxonomy'] ) ) {
						...
						$term_output = apply_filters( 'generate_dynamic_text_post_terms', $term_output, $block );
				...
				if ( 'comments-number' === $text_type ) {
					if ( ! post_password_required( $id ) && ( comments_open( $id ) || get_comments_number( $id ) ) ) {
						...
						$comments_text = apply_filters( 'generate_dynamic_text_comments_number', $comments_text, $block );
				...
				if ( 'post-meta' === $text_type || 'term-meta' === $text_type || 'user-meta' === $text_type ) {
					if ( ! empty( $block['attrs']['gpDynamicTextCustomField'] ) ) {
						...
						if ( 'term-meta' === $text_type ) {
							$custom_field = get_post_meta( get_queried_object_id(), $block['attrs']['gpDynamicTextCustomField'], true );
						}
						...
						if ( $custom_field ) {
							...
							$custom_field = apply_filters( 'generate_dynamic_text_custom_field', $custom_field, $block );
