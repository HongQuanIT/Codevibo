...
function addControls( output, id, props, state ) {
	...
	const conditionOptions = [
		...
		{
			label: __( 'No featured image', 'gp-premium' ),
			value: 'no-featured-image',
		},
		{
			label: __( 'No post meta', 'gp-premium' ),
			value: 'no-post-meta',
		},
	];
	...
	if ( 'post-navigation-template' === meta._generate_block_type ) {
		conditionOptions.push( {
			label: __( 'No next post', 'gp-premium' ),
			value: 'no-next-post',
		} );

		conditionOptions.push( {
			label: __( 'No previous post', 'gp-premium' ),
			value: 'no-previous-post',
		} );
	}
	...
	return (
		<Fragment>
			...
			<Fragment>
				...
				{ 'no-post-meta' === gpRemoveContainerCondition &&
					<TextControl
						label={ __( 'Meta field name', 'gp-premium' ) }
						type={ 'text' }
						value={ gpRemoveContainerConditionPostMeta }
						onChange={ ( value ) => {
							setAttributes( {
								gpRemoveContainerConditionPostMeta: value,
							} );
						} }
					/>
				}
