...
class BlockElementSettings extends Component {
	...
	componentDidMount() {
		...
		// Show the Page Attributes panel with Content Templates only.
		if ( 'undefined' !== wp.data.select( 'core/edit-post' ).getPreference( 'panels' )[ 'page-attributes' ] ) {
			const pageAttributesPanel = wp.data.select( 'core/edit-post' ).getPreference( 'panels' )[ 'page-attributes' ];

			if ( 'undefined' !== typeof pageAttributesPanel && pageAttributesPanel.enabled ) {
				wp.data.dispatch( 'core/edit-post' ).toggleEditorPanelEnabled( 'page-attributes' );
			}
		}

		if ( !! this.props.elementParent ) {
			document.body.classList.add( 'element-has-parent' );
		}
	}
	...
	componentDidUpdate() {
		...
		if ( ( !! this.props.elementParent || !! meta._generate_post_loop_item_display ) && ! document.body.classList.contains( 'element-has-parent' ) ) {
			document.body.classList.add( 'element-has-parent' );
		} else if ( ( ! this.props.elementParent || ! meta._generate_post_loop_item_display ) && document.body.classList.contains( 'element-has-parent' ) ) {
			document.body.classList.remove( 'element-has-parent' );
		}
	}
	...
	render() {
		...
		const elementParentOptions = [];

		const elementParents = gpPremiumBlockElements.parentElements;

		if ( elementParents ) {
			Object.keys( elementParents ).forEach( ( element ) => {
				elementParentOptions.push( {
					label: elementParents[ element ].label,
					value: elementParents[ element ].id,
				} );
			} );
		}
						...
						<Fragment>
							...
							<SelectControl
								...
								options={ [
									...
									{ label: __( 'First post', 'gp-premium' ), value: 'is-first-post' },
								] }
								onChange={ ( value ) => {
							...
							{ '' !== postLoopItemDisplay &&
								<Fragment>
									...
									{ 'has-post-meta' === postLoopItemDisplay &&
										<TextControl
									...
									{ elementParentOptions.length > 0 &&
										<Fragment>
											<BaseControl>
												<ComboboxControl
													className="editor-page-attributes__parent"
													label={ __( 'Parent Element', 'gp-premium' ) }
													help={ __( 'Set a parent Element to inherit in case the above conditions do not apply.', 'gp-premim' ) }
													value={ elementParent }
													options={ elementParentOptions }
													onFilterValueChange={ ( value ) => {
														if ( value ) {
															debounce( updateParentId( value ), 300 );
														}
													} }
													onChange={ ( value ) => {
											...
											{ !! elementParent &&
												<TextControl
													type="number"
													label={ __( 'Display Order', 'gp-premium' ) }
													onChange={ ( value ) => {
														updateOrder( value );
													} }
													value={ elementMenuOrder }
												/>
											}
										</Fragment>
									}
...
const applyWithSelect = withSelect( ( select ) => {
	...
	return {
		...
		elementParent: getEditedPostAttribute( 'parent' ),
		elementMenuOrder: getEditedPostAttribute( 'menu_order' ),
	};
} );
